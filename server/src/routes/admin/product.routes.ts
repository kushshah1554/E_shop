...
adminProductRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    requireFound(products, "Products not found", 404);
...
adminProductRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const images: UploadedImage[] = uploadedImages.map((img, index) => ({
      url: img.url,
...
adminProductRouter.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const newlyAddedImages: UploadedImage[] = uploadNewImages.map((image) => ({
      url: image.url,
